...
const Welcome = (props) => { 
	...
	return (
		<Panel id={props.id} centered={false}>
			<PanelHeader>Добро пожаловать, {(props.session.vkUser ? props.session.vkUser.first_name : "Неизвестный")}!</PanelHeader>
	{/* 		<Div>
	...
	*/}		
			{userInfo && parseInt(userInfo.is_blocked) === 0 &&
				<Group>
					<Div>
						<Button size="l" mode="primary" stretched={true} onClick={props.session.go} data-to="indicationsview.accountslist">
							Перейти к вводу показаний
						</Button>
					</Div>
					<Div>
						<Button size="l" mode="primary" stretched={true} onClick={props.session.go} data-to="registrationview.registrationlist">
							Присоединить лицевой счет
						</Button>
					</Div>
					{['ADMIN', 'OPERATOR'].indexOf(userInfo.privileges) != -1 && 
						<Div>
							<Button size="l" mode="primary" stretched={true} onClick={props.session.go} data-to="adminview.lobby">
								Администрирование
							</Button>
						</Div>
					}
			...
			<Div>
				<Button size="l" mode="primary" stretched={true} onClick={props.session.go} data-to="welcomeview.help">
					Помощь
				</Button>
			</Div>
			{userInfo && parseInt(userInfo.is_blocked) !== 0 &&
